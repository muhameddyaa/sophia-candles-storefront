...
import { ArrowLeft, ArrowRight, ArrowUpRight, ChevronRight, CreditCard, MapPin, Minus, PackageCheck, Plus, Search, ShoppingBag, Trash2, UserRound, X } from "lucide-react";
...
type Product = { id: string; category: Exclude<Category, "all">; image: string; tag: string; title: string; subtitle: string; price?: number; originalPrice?: number; badge?: string; temporary?: boolean };
type CartItem = { id: string; quantity: number };

const products: Record<Language, Product[]> = {
  en: [
    { id: "rose-note", category: "bouquets", image: images.heartBouquet, tag: "FLOWER BOUQUET", title: "Rose Note Candle Bouquet", subtitle: "A candle bouquet with space for your message.", price: 140, originalPrice: 180, badge: "AED 40 SAVING" },
    { id: "pastel-wrap", category: "bouquets", image: images.pastelWrap, tag: "FLOWER BOUQUET", title: "Pastel Wrap Candle Bouquet", subtitle: "A soft floral candle bouquet finished in ribbon wrap.", price: 140, originalPrice: 180, badge: "AED 40 SAVING" },
    { id: "rose-box", category: "bouquets", image: images.roseBox, tag: "SMALL BOUQUET", title: "Rose Box Candle Bouquet", subtitle: "A compact bloom arrangement in a keepsake box.", price: 100, originalPrice: 150, badge: "AED 50 SAVING" },
    { id: "pink-bloom", category: "bouquets", image: images.pinkBloom, tag: "SMALL BOUQUET", title: "Pink Bloom Candle Bouquet", subtitle: "Soft rose candles gathered in a small floral arrangement.", price: 100, originalPrice: 150, badge: "AED 50 SAVING" },
    { id: "lilac-bloom", category: "bouquets", image: images.lilacCloseup, tag: "SMALL BOUQUET", title: "Lilac Bloom Candle Bouquet", subtitle: "Pastel candle flowers in a soft gifting wrap.", price: 100, originalPrice: 150, badge: "AED 50 SAVING" },
    { id: "teddy-gift", category: "candles", image: images.teddyGift, tag: "CANDLE", title: "Teddy Bloom Gift Candle", subtitle: "A handmade keepsake in a clear gift box.", price: 200, temporary: true },
    { id: "teddy-bowl", category: "candles", image: images.teddyBowl, tag: "CANDLE", title: "Teddy Bloom Bowl Candle", subtitle: "A soft sculptural candle scene for gifting.", price: 200, temporary: true },
    { id: "blue-favors", category: "favors", image: images.blueFavors, tag: "FAVORS", title: "Blue Bloom Celebration Favors", subtitle: "Small ribbon-tied pieces for a special table.", price: 200, temporary: true },
  ],
  ar: [
    { id: "rose-note", category: "bouquets", image: images.heartBouquet, tag: "باقات ورود", title: "باقة شموع ورد مع بطاقة", subtitle: "باقة شموع مع مساحة لرسالتك.", price: 140, originalPrice: 180, badge: "وفر AED 40" },
    { id: "pastel-wrap", category: "bouquets", image: images.pastelWrap, tag: "باقات ورود", title: "باقة شموع ورد باستيل", subtitle: "باقة ورد شموع ناعمة بتغليف وشريط رقيق.", price: 140, originalPrice: 180, badge: "وفر AED 40" },
    { id: "rose-box", category: "bouquets", image: images.roseBox, tag: "باقة صغيرة", title: "باقة شموع ورد في علبة", subtitle: "تنسيق ورد شمعي صغير داخل علبة مميزة.", price: 100, originalPrice: 150, badge: "وفر AED 50" },
    { id: "pink-bloom", category: "bouquets", image: images.pinkBloom, tag: "باقة صغيرة", title: "باقة شموع ورد وردية", subtitle: "ورد شمعي ناعم متجمع في تنسيق صغير.", price: 100, originalPrice: 150, badge: "وفر AED 50" },
    { id: "lilac-bloom", category: "bouquets", image: images.lilacCloseup, tag: "باقة صغيرة", title: "باقة شموع ورد ليلكي", subtitle: "ورود شموع باستيل في تغليف هدايا ناعم.", price: 100, originalPrice: 150, badge: "وفر AED 50" },
    { id: "teddy-gift", category: "candles", image: images.teddyGift, tag: "شموع", title: "شمعة Teddy Bloom في علبة هدايا", subtitle: "قطعة يدوية مميزة داخل علبة هدايا شفافة.", price: 200, temporary: true },
    { id: "teddy-bowl", category: "candles", image: images.teddyBowl, tag: "شموع", title: "شمعة Teddy Bloom في وعاء", subtitle: "مشهد شموع فني ناعم للهدية.", price: 200, temporary: true },
    { id: "blue-favors", category: "favors", image: images.blueFavors, tag: "توزيعات", title: "توزيعات Blue Bloom", subtitle: "قطع صغيرة بشريط رقيق لسفرة مناسبتك.", price: 200, temporary: true },
  ],
};
...
const copy = {
  en: {
    ...
    nav: "Shop all", categoryIntro: "SHOP BY CATEGORY", categoryTitle: "Gifts, made to hold the moment.", categoryButton: "Explore", categories: [
    ...
    bestKicker: "THE SOPHIA EDIT", bestTitle: "Gifts kept close", bestSub: "Four hand-finished gestures in wax, ribbon and flowers.", allProducts: "Shop all products", add: "Add to bag", unavailable: "Price coming soon", request: "Enquire", temporaryPrice: "TEMPORARY PRICE [ESTIMATE]", offerKicker: "FOR A SHORT WHILE", offerTitle: "Bouquets, softly priced", offerSub: "Hand-finished blooms, with a gentler price for now.", viewOffers: "See bouquet offers", was: "was", checkout: "Proceed to checkout", cartTitle: "Your bag", cartEmpty: "Your bag is waiting for something beautiful.", cartEmptyCta: "Explore bouquets", subtotal: "Subtotal", cartNote: "Delivery and checkout will be confirmed next.", checkoutToast: "Checkout is ready to connect", checkoutDescription: "Please confirm the Shopify store to activate secure checkout and payments.", added: "Added to your bag", unavailableToast: "This price is being prepared", unavailableDescription: "We will add this product to the cart once its AED price is confirmed.", checkoutKicker: "ORDER PREVIEW", checkoutTitle: "Almost yours.", contact: "Contact details", deliveryTitle: "Delivery details", payment: "Payment", fullName: "Full name", checkoutEmail: "Email address", phone: "Mobile number", address: "Delivery address", emirate: "Emirate", paymentNote: "Secure payment options will appear here after Shopify is connected.", deliveryNote: "Delivery availability and timing will be confirmed at checkout.", orderSummary: "Your order", checkoutDisabled: "Checkout unlocks with Shopify", previewNotice: "Preview only — details entered here are not saved or submitted.",
  ...
  ar: {
    ...
    nav: "كل المنتجات", categoryIntro: "تسوقي حسب الفئة", categoryTitle: "هدايا، مصنوعة لتحفظ معنى اللحظة.", categoryButton: "اكتشفي", categories: [
    ...
    bestKicker: "اختيارات SOPHIA", bestTitle: "هدايا تفضل قريبة", bestSub: "أربع لفتات مصنوعة بالشمع والشريط والورد.", allProducts: "كل المنتجات", add: "أضيفي للحقيبة", unavailable: "السعر قريباً", request: "استفسري", temporaryPrice: "سعر مؤقت [ESTIMATE]", offerKicker: "لفترة قصيرة", offerTitle: "باقات بسعر ألطف", offerSub: "باقات مصنوعة يدوياً بسعر أهدى لفترة محدودة.", viewOffers: "شاهدي عروض الباقات", was: "بدلاً من", checkout: "إتمام الطلب", cartTitle: "حقيبتك", cartEmpty: "حقيبتك مستنية حاجة حلوة.", cartEmptyCta: "شاهدي الباقات", subtotal: "المجموع", cartNote: "التوصيل وإتمام الطلب هيتأكدوا في الخطوة التالية.", checkoutToast: "الـcheckout جاهز للربط", checkoutDescription: "أكدي متجر Shopify عشان نفعل الدفع والـcheckout الآمن.", added: "اتضاف للحقيبة", unavailableToast: "بنجهز السعر حالياً", unavailableDescription: "هنضيف المنتج للسلة بعد تأكيد سعره بالـAED.", checkoutKicker: "معاينة الطلب", checkoutTitle: "باقي خطوة واحدة.", contact: "بيانات التواصل", deliveryTitle: "بيانات التوصيل", payment: "الدفع", fullName: "الاسم بالكامل", checkoutEmail: "البريد الإلكتروني", phone: "رقم الموبايل", address: "عنوان التوصيل", emirate: "الإمارة", paymentNote: "هتظهر اختيارات الدفع الآمنة هنا بعد ربط Shopify.", deliveryNote: "هيتأكد التوصيل والموعد في الـcheckout.", orderSummary: "طلبك", checkoutDisabled: "الـcheckout هيتفعّل مع Shopify", previewNotice: "معاينة فقط — البيانات هنا لا تُحفظ ولا تُرسل.",
...
export default function Home() {
  ...
  return (
    <div className="sophia-shop" dir={t.dir} data-lang={language}>
      ...
      <CheckoutPreview open={checkoutOpen} onOpenChange={setCheckoutOpen} language={language} t={t} rows={cartRows} total={total} />
    </div>
  );
}

function ProductCard({ product, t, addToCart }: { product: Product; t: (typeof copy)[Language]; addToCart: (product: Product) => void }) {
  return <article className="best-card"><div className="best-image"><img src={product.image} alt={product.title} /><span>{product.tag}</span>{product.badge && <b>{product.badge}</b>}</div><div className="best-details"><p>{product.tag}</p><h3>{product.title}</h3><span>{product.subtitle}</span>{product.price ? <div className="price-row"><b>{money(product.price)}</b>{product.originalPrice && <s>{money(product.originalPrice)}</s>}{product.temporary && <small>{t.temporaryPrice}</small>}</div> : <div className="price-row unavailable"><b>{t.unavailable}</b></div>}<button onClick={() => addToCart(product)}>{product.price ? t.add : t.request}<ShoppingBag size={14} /></button></div></article>;
}

function CheckoutPreview({ open, onOpenChange, language, t, rows, total }: { open: boolean; onOpenChange: (open: boolean) => void; language: Language; t: (typeof copy)[Language]; rows: Array<CartItem & { product: Product }>; total: number }) {
  const input = (label: string, type = "text") => <label><span>{label}</span><input type={type} placeholder={label} /></label>;
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent side={language === "ar" ? "left" : "right"} className="checkout-sheet"><SheetHeader><p>{t.checkoutKicker}</p><SheetTitle>{t.checkoutTitle}</SheetTitle><SheetDescription>{t.previewNotice}</SheetDescription></SheetHeader><form onSubmit={(event) => { event.preventDefault(); toast(t.checkoutToast, { description: t.checkoutDescription }); }} className="checkout-form"><section><h3><UserRound size={16} />{t.contact}</h3><div className="field-grid">{input(t.fullName)}{input(t.checkoutEmail, "email")}<span className="full-field">{input(t.phone, "tel")}</span></div></section><section><h3><MapPin size={16} />{t.deliveryTitle}</h3><div className="field-grid">{input(t.address)}{input(t.emirate)}<span className="full-field checkout-note"><PackageCheck size={16} />{t.deliveryNote}</span></div></section><section><h3><CreditCard size={16} />{t.payment}</h3><div className="payment-placeholder"><CreditCard size={19} /><span>{t.paymentNote}</span></div></section><section className="checkout-summary"><h3>{t.orderSummary}</h3>{rows.map(({ product, quantity }) => <div key={product.id}><span>{product.title} × {quantity}{product.temporary && <small>{t.temporaryPrice}</small>}</span><b>{product.price && money(product.price * quantity)}</b></div>)}<div className="checkout-total"><span>{t.subtotal}</span><b>{money(total)}</b></div></section><button type="submit" className="checkout-disabled">{t.checkoutDisabled}<ArrowUpRight size={16} /></button></form></SheetContent></Sheet>;
}
